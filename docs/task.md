...
### Цель: Получить практический опыт построения инфраструктуры "как код" (IaC), 
настройки CI/CD пайплайна, контейнеризации приложения, обеспечения мониторинга 
и отказоустойчивости.

### Итоговый результат:

Практикант должен создать в облачном провайдере (Yandex Cloud, AWS, GCP или на 
локальной виртуализации) инфраструктуру для работы веб-приложения (например, 
простой блог на WordPress или API на Python/Go), которая будет:
...
Действия:
Зарегистрироваться в облачном провайдере (или развернуть локально 3-4 виртуальные машины через KVM/VirtualBox).
Создать 2-3 инстанса (виртуальные машины) с ОС на выбор, предпочтительно выбрать отечественную ОС.
Настроить SSH-доступ с использованием ключевой аутентификации.
Обновить системы, настроить hostname, статичные адреса (или DHCP), файрвол (ufw), отключить вход по паролю.
На одной VM установить и настроить Nginx/Apache, на другой — базу данных (MySQL/PostgreSQL). Обеспечить сетевое взаимодействие между ними.

Критерии проверки: Предоставлен доступ к VM, проверена настройка безопасности и работоспособность веб-сервера и БД.
